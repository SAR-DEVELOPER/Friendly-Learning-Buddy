...
type SubcategoryKey = 'critical-thinking' | 'books' | 'ielts'

const subcategoryConfig: Record<SubcategoryKey, { title: string; description: string }> = {
  'critical-thinking': {
    title: 'Critical Thinking',
    description: 'Frameworks and mental models for sharper analysis, clearer reasoning, and better decisions.',
  },
  books: {
    title: 'Books',
    description: 'Key readings and summaries across finance, economics, strategy, and policy.',
  },
  ielts: {
    title: 'IELTS',
    description: 'Preparation strategies, practice resources, and study frameworks for the IELTS examination.',
  },
}

type Props = { params: Promise<{ subcategory: string }> }

export default async function LearningSubcategoryPage({ params }: Props) {
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="container mx-auto px-4 md:px-8 lg:px-16 pt-24 pb-4">
        <div className="max-w-4xl mx-auto">
          <Breadcrumb items={[{ label: 'Learning' }, { label: config.title }]} />
        </div>
      </div>
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const config = subcategoryConfig[subcategory as SubcategoryKey]
  return {
    title: `${config.title} - Learning | Friendly Learning Buddy`,
